...
class TestDriverGenerator:
    ...
    def write_list_vars_for_cmake(self, fname: str) -> None:
        ...
        def to_cmake_path(path: Path) -> str:
            # CMake list files should use forward slashes, otherwise Windows
            # backslashes can be parsed as escape sequences (e.g. "\b").
            return path.as_posix()
        with open(self.dst_dir / fname, "w") as f:
            f.write(f"set({self.driver}_input_files\n    " + \
                     "\n    ".join(f'"{to_cmake_path(path)}"' for path in src_relpaths) + "\n)\n\n")
            f.write(f"set({self.driver}_files\n    " + \
                    "\n    ".join(f'"{to_cmake_path(self.__get_dst_relpath(path.relative_to(self.src_dir)))}"' \
