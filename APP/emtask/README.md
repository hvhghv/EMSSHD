...
- 可开启一个全局面板端口，通过 HTTP/JSON 查询所有任务配置、监听端口和运行状态，并支持 token、OTP 或 token+OTP 鉴权；也可由 Flutter 面板动态添加子任务，并用 SQLite 持久化。
...
- `panel_auth_file` 与 `panel_qr_file` 都包含或间接包含面板密钥材料；不要提交到源码仓库，也不要暴露到公网静态目录。若需要避免旧二维码和新鉴权材料不一致，保持默认 `panel_qr_mode = always`。
